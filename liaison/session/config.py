...
def _raise_req_error(key, value, dict_trace, prefix_msg=''):
  ...
  raise ConfigError('{}{}must be {}.'.format(prefix_msg,
                                             _trace_key(dict_trace, key), msg))
    ...
    if key not in config:
      if _is_req(default_value):
        ...
        if _has_req(default_value):
          raise ConfigError(
              'Sub-dict under key "{}" contains a required config: {}.'.format(
                  _trace_key(dict_trace, key), default_value))
      ...
      if _is_req(default_value):
        ...
        if _is_req(value):
          if value != default_value:
            raise ConfigError(
                'inherited {}: "{}" must match default "{}"'.format(
                    _trace_key(dict_trace, key), value, default_value))
        else:
          checker = _req_type_check(default_value)
          if not checker(value):
            _raise_req_error(key, default_value, dict_trace, 'Wrong type:')
      else:
...
_RESERVED_METHODS = [
    'keys', 'items', 'values', 'get', 'copy', 'update', 'extend', 'load_file',
    'dump_file', 'to_dict'
]
...
class Config(dict):
  ...
  def __setattr__(self, name, value):
    if name in _RESERVED_METHODS:
      raise ConfigError(
          '"{}()" is a reserved method, cannot override'.format(name))
  ...
  def to_dict(self):
    ...
    for k, value in self.items():
      if isinstance(value, Config):
        ...
        d[k] = type(value)(v.to_dict() if isinstance(v, Config) else v
                           for v in value)
      else:
        d[k] = value
    return d
  ...
  def dump_file(self, file_path):
    assert file_path.endswith('.json') or file_path.endswith(
        'yaml') or file_path.endswith('yml')
