...
parser.add_argument('-Tr', '--n_training_samples', type=int, default=0)
parser.add_argument('-V', '--n_valid_samples', type=int, default=0)
parser.add_argument('-Te', '--n_test_samples', type=int, default=0)
...
def compute_score(fname):
  ...
  if sample:
    ...
    size = sample['problem_size']
  else:
    score, size = None, None
  return (score, size, fname)
...
def _filter():
  ...
  l = list(filter(lambda k: k[0], scores))
  if args.max_size:
    l = list(filter(lambda k: k[1] <= args.max_size, l))
  return l
...
def sort_dataset(dataset):
  size_map = {}
  for sample in sorted(dataset, reverse=False):
    k = sample[1]
    if k in size_map:
      size_map[k].append(sample)
    else:
      size_map[k] = [sample]

  l = zip(*[size_map[key] for key in sorted(size_map)])
  # flatten the list
