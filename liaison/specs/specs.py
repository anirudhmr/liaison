...
class ArraySpec(object):
  ...
  def validate(self, value):
    ...
    if value.shape != self.shape:
      self._fail_validation('Expected shape %r but found %r', self.shape,
                            value.shape)
    if value.dtype != self.dtype:
      self._fail_validation('Expected dtype %s but found %s', self.dtype,
                            value.dtype)
  ...
  def expand_dims(self, n_repeat=1, axis=0):
...
class BoundedArraySpec(ArraySpec):
  ...
  def __eq__(self, other):
    if not isinstance(other, BoundedArraySpec):
      return False
    return (super(BoundedArraySpec, self).__eq__(other)
  ...
  def validate(self, value):
    ...
    if (value < self.minimum).any() or (value > self.maximum).any():
      self._fail_validation(
          'Values were not all within bounds %s <= value <= %s', self.minimum,
          self.maximum)
