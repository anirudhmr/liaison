...
def report_exitcode(code, name='process'):
  ...
  if code == 0:
    ...
    raise ValueError('[Warning] {} exited with exit code {}'.format(
        name, code))
  else:
...
def assert_type(x, expected_type, message=''):
  assert isinstance(
      x, expected_type), ((message + ': ' if message else '') +
...
def fformat(float_num, precision):
  ...
  return (
      '{{:.{}f}}'.format(precision).format(float_num).rstrip('0').rstrip('.'))
...
def meta_wrap(decor):
  """
    ...
    """
  single_callable = (lambda args, kwargs: len(args) == 1 and len(kwargs) == 0
                     and callable(args[0]))
...
class ArgParser(object):
  ...
  def add_boolean_flag(self, name, default=False, pair=True, help=None):
    ...
    self.parser.add_argument("--" + name,
                             ...
                             help=help)
    if pair:
      self.parser.add_argument("--no-" + name, action="store_false", dest=name)
        ...
        if ret == 0:
          ...
          for process in processes:
            if process is not None:
              process.kill()
          raise RuntimeError('Process {} exited with code {}'.format(i, ret))
