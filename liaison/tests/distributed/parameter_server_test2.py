...
from distributed import (ParameterClient, ParameterPublisher,
                         ShardedParameterServer, ParameterServer, Shell)
...
class ParameterServerTest(tf.test.TestCase):
  ...
  def _get_ps(self):
    return ParameterServer(publisher_host=SYMPH_PARAMETER_PUBLISH_HOST,
                           publisher_port=SYMPH_PARAMETER_PUBLISH_PORT,
                           serving_host='localhost',
                           serving_port=SYMPH_PS_FRONTEND_PORT,
                           load_balanced=False,
                           supress_output=True)
